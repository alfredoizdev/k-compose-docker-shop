...
export const orderResolvers = {
  Query: {
    orders: async (_: any, _arg: any, { token }: MyContext): Promise<any> => {
  ...
  Mutation: {
    addOrder: async (
      _: any,
      { numberOfItems, orderItems, subTotal, tax, total }: IOrder,
      { token }: MyContext,
    ): Promise<IOrder> => {
    ...
    updateOrder: async (_: any, { numberOfItems, orderItems, subTotal, tax, total }: IOrder, { token }: MyContext) => {
      if (!token) throw new Error('This Action is not allowed');
    ...
    deleteOrder: async (_: any, { token }: MyContext): Promise<IOrder> => {
      if (!token) throw new Error('This Action is not allowed');
