...
export const userResolvers = {
  ...
  Mutation: {
    addUser: async (_: any, { name, last, email, password, role = 'user' }: IUser): Promise<string> => {
    ...
    updateUser: async (_: any, { name, last, email, role }: IUser, { token }: MyContext) => {
      if (!token) throw new Error('Action not allowed');
    ...
    deleteUser: async (_: any, { id }: IUser, { token }: MyContext): Promise<IUser> => {
